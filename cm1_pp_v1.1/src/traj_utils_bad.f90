...
contains
  ...
  !#    real,dimension(:,:,:) buoy : buoyancy field               #!
  !#    real,dimension(:,:,:) pgfx, pgfy, pgfz : total PGFs       #!
  !#    real,dimension(:,:,:) tke : turbulent kinetic energy      #!
  !#    real,dimension(:) thr0 : base state density potential     #!
  !#      temperature                                             #!
  !#    real,dimension(:,:,:) thr : total density potential temp  #!
  !#    real,dimension(:,:,:) qv, qc, qr, qi, qs, qg, qhl :       #!
  !#      mixing ratios                                           #!
  !#    real,dimension(:,:,:) ncc, ncr, nci, ncs, ncg :           #!
  ...
  !#    real buoyp : buoyancy of parcel                           #!
  !#    real pgfxp, pgfyp, pgfzp : PGF at parcel position         #!
  !#    real tkep : TKE at parcel position                        #!
  !#    real thrp : perturbation density potential temperature of #!
  !#      parcel                                                  #!
  !#    real qvp, qcp, qrp, qip, qsp, qgp, qhl : mixing ratios of #!
  !#      parcel                                                  #!
  !#    real ncp, nrp, nip, nsp, ngp : number concentration at    #!
  !#      parcel position                                         #!
  !#    real rhodp : dry air density of parcel                    #!
  !#    real thp : potential temperature of parcel                #!
  !#    real pip : nondimensionalized pressure of parcel          #!
  !#==============================================================#!
  !# v1.0, Ryan Hastings 13 December 2016                         #!
  !################################################################!
  subroutine interp_all( xpc, ypc, zpc, zzh, t1, t2, ptime, &
    u1, u2, up, v1, v2, vp, w1, w2, wp, buoy1, buoy2, buoyp, &
    pgfx1, pgfx2, pgfxp, pgfy1, pgfy2, pgfyp, pgfz1, pgfz2, pgfzp, &
    tke1, tke2, tkep, thr0, thr1, thr2, thrp, &
    qv1, qv2, qvp, qc1, qc2, qcp, qr1, qr2, qrp, qi1, qi2, qip, &
    qs1, qs2, qsp, qg1, qg2, qgp, qhl1, qhl2, qhlp, &
    ncc, ncp, ncr, nrp, nci, nip, ncs, nsp, ncg, ngp, rhod1, rhod2, rhodp, &
    th1, th2, thp, ppi1, ppi2, pip )
    ...
    real,intent(in),dimension(1:ni+1,1:nj,0:nk) :: u1, u2, pgfx1, pgfx2
    real,intent(in),dimension(1:ni,1:nj+1,0:nk) :: v1, v2, pgfy1, pgfy2
    real,intent(in),dimension(1:ni,1:nj,1:nk+1) :: w1, w2, tke1, tke2, pgfz1, pgfz2

    real,intent(in),dimension(1:ni,1:nj,0:nk) :: buoy1, buoy2, thr1, thr2, th1, th2
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: ppi1, ppi2
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: qv1, qv2, qc1, qc2, qr1, qr2
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: qi1, qi2, qs1, qs2, qg1, qg2, 
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: qhl1, qhl2, rhod1, rhod2
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: ncc, ncr, nci, ncs, ncg
    real,intent(in),dimension(1:nk) :: thr0

    real,intent(in),dimension(0:nk) :: zzh

    ! Returned variables
    real,intent(out) :: up, vp, wp, buoyp, pgfxp, pgfyp, pgfzp, tkep, thrp
    real,intent(out) :: qvp, qcp, qrp, qip, qsp, qgp, qhlp
    real,intent(out) :: ncp, nrp, nip, nsp, ngp, rhodp, thp, pip

    ! Local variables
    integer :: ih, iif, jh, jf, kh, kf
    real :: dum_thr, dum_thr0
    real,dimension(0:nk):: thr0_2
    ...
    call interp_u( u1, u2, t1, t2, ptime, xpc, ypc, zpc, iif, jh, kh, zzh, up )
    call interp_u( pgfx1, pgfx2, t1, t2, ptime, xpc, ypc, zpc, iif, jh, kh, zzh, pgfxp )
    call interp_v( v1, v2, t1, t2, ptime, xpc, ypc, zpc,  ih, jf, kh, zzh, vp )
    call interp_v( pgfy1, pgfy2, t1, t2, ptime, xpc, ypc, zpc,  ih, jf, kh, zzh, pgfyp )
    call interp_w( w1, w2, t1, t2, ptime, xpc, ypc, zpc,  ih, jh, kf, wp )
    call interp_w( pgfz1, pgfz2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kf, pgfzp )

    call interp_scalars( buoy1, buoy2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, buoyp )
    call interp_w( tke1, tke2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kf, tkep )
    call interp_scalars( thr1, thr2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, dum_thr )

    call interp_scalars( qv1, qv2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qvp )
    call interp_scalars( qc1, qc2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qcp )
    call interp_scalars( qr1, qr2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qrp )
    call interp_scalars( qi1, qi2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qip )
    call interp_scalars( qg1, qg2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qgp )
    call interp_scalars( qs1, qs2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, qsp )
!    if(ptype.eq.27)then
!      call interp_scalars( qhl, xpc, ypc, zpc, ih, jh, kh, zzh, qhlp )
!    endif

!    call interp_scalars( ncc, xpc, ypc, zpc, ih, jh, kh, zzh, ncp )
!    call interp_scalars( ncr, xpc, ypc, zpc, ih, jh, kh, zzh, nrp )
!    call interp_scalars( nci, xpc, ypc, zpc, ih, jh, kh, zzh, nip )
!    call interp_scalars( ncs, xpc, ypc, zpc, ih, jh, kh, zzh, nsp )
!    call interp_scalars( ncg, xpc, ypc, zpc, ih, jh, kh, zzh, ngp )

!    call interp_scalars( rhod, xpc, ypc, zpc, ih, jh, kh, zzh, rhodp )
!    call interp_scalars( th, xpc, ypc, zpc, ih, jh, kh, zzh, thp )
!    call interp_scalars( ppi, xpc, ypc, zpc, ih, jh, kh, zzh, pip )

    thr0_2(1:nk) = thr0
    thr0_2(0) = thr0(1)

    dum_thr0 = thr0_2(kh) + (zpc-zzh(kh))*(thr0_2(kh+1)-thr0_2(kh))/(zzh(kh+1)-zzh(kh))

    thrp=dum_thr-dum_thr0
  ...
  !################################################################!
  subroutine interp_winds( xpc, ypc, zpc, zzh, u1, u2, up, v1, v2, vp, w1, w2, wp, t1, t2 ptime )
    ...
    call interp_u( u1, u2, t1, t2, ptime, xpc, ypc, zpc, iif, jh, kh, zzh, up )
    call interp_v( v1, v2, t1, t2, ptime, xpc, ypc, zpc,  ih, jf, kh, zzh, vp )
    call interp_w( w1, w1, t1, t2, ptime, xpc, ypc, zpc,  ih, jh, kf, wp )

  end subroutine

  subroutine interp1_scalar(x1,x2,y1,y2,x,y)
    implicit none
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: y1,y2
    real,intent(in) :: x1,x2
    real,intent(out),dimension(1:ni,1:nj,0:nk) :: y
  ...
  !# v1.0, Ryan Hastings, 13 Dec 2016                             #!
  !################################################################!
  subroutine interp_u( u1, u2, t1, t2, ptime, xpc, ypc, zpc, iif, jh, kh, zzh, up )

    implicit none

    real,intent(in),dimension(1:ni+1,1:nj,0:nk) :: u1, u2, u
    real,intent(in) :: xpc, ypc, zpc, t1, t2, ptime
  ...
  !# v1.0, Ryan Hastings, 13 Dec 2016                             #!
  !################################################################!
  subroutine interp_v( v1, v2, t1, t2, ptime, xpc, ypc, zpc, ih, jf, kh, zzh, vp )

    implicit none

    real,intent(in),dimension(1:ni,1:nj+1,0:nk) :: v1, v2, v
  ...
  !# v1.0, Ryan Hastings, 13 Dec 2016                             #!
  !################################################################!
  subroutine interp_w( w1, w2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kf, wp )

    implicit none

    real,intent(in),dimension(1:ni,1:nj,1:nk+1) :: w1, w2, w
    real,intent(in) :: xpc, ypc, zpc, t1, t2, ptime
  ...
  !# v1.0, Ryan Hastings, 13 Dec 2016                             #!
  !################################################################!
  subroutine interp_scalars( scalar1, scalar2, t1, t2, ptime, xpc, ypc, zpc, ih, jh, kh, zzh, scalarp )


    implicit none

    real,intent(in),dimension(1:ni,1:nj,0:nk) :: scalar1, scalar2, scalar
    real,intent(in) :: xpc, ypc, zpc, t1, t2, ptime
    ...
    scalar=scalar1+(t2-ptime)*(scalar2-scalar1)/(t2-t1)
  ...
  !# v1.0, Ryan Hastings, 13 Dec 2016                             #!
  !################################################################!
  subroutine interp_dynz( xpc, ypc, zpc, t1, t2, ptime, &
    pgfx_b1, pgfx_b2, pgfxbp, pgfy_b1, pgfy_b2, pgfybp, &
    pgfz_b1  pgfz_b1, pgfzbp, &
    pgfx_dl1, pgfx_dl2, pgfxdlp, pgfy_dl1, pgfy_d2, pgfydlp, &
    pgfz_dl1, pgfz_dl2, pgfzdlp, &
    pgfx_dn1, pgfx_dn1, pgfxdnp, pgfy_dn1, pgfy_dn2, pgfydnp, &
    pgfz_dn1, pgfz_dn2, pgfzdnp )
