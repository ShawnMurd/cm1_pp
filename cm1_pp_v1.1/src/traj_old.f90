program traj
  ...
  real,dimension(:,:,:),allocatable :: rhod ! dry air density (kg/m^3)
  real,dimension(:,:,:),allocatable :: ncc ! number concentration of cloud drops (m^-3)
  real,dimension(:,:,:),allocatable :: ncr ! number concentration of rain drops
  real,dimension(:,:,:),allocatable :: nci ! number concertration of ice particles
  real,dimension(:,:,:),allocatable :: ncs ! number concertration of snowflakes
  real,dimension(:,:,:),allocatable :: ncg ! number concentration of graupel
  real,dimension(:,:,:),allocatable :: ppi ! nondimensionalized pressure
  ...
  real,dimension(:,:,:),allocatable :: tke ! tubulent kinetic energy
    ...
    do tstep=start_time,ntimes-1
      ...
      do np=1,nparcels
        ...
        call interp_all( xpc(tstep,np), ypc(tstep,np), zpc(tstep,np), zzh, u, up(tstep,np), v, vp(tstep,np), &
          ...
          ncs, nsp(tstep,np), ncg, ngp(tstep,np), rhod, rhodp(tstep,np), th, thp(tstep,np),&
          ppi, pip(tstep,np) )
