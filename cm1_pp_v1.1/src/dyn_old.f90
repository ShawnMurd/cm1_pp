program dyn
  ...
  !####---------------------------------------------------------------------####!
  !#### SSM (10/5/2020): Revised code to explicitly compute p'dn            ####!
  !#############################################################################!
  !#############################################################################!
  ...
  real,dimension(:,:,:),allocatable :: uipert, vipert
  ...
  real,dimension(:,:,:),allocatable :: dum1, dum2, dum3 ! terms in nonlinear dynamic p' forcing function
  real,dimension(:,:,:),allocatable :: dwdx, dwdy, dwdz
  real,dimension(:,:,:),allocatable :: dudy, dudx, dvdx, dvdy
  real,dimension(:,:,:),allocatable :: wterms, div
...
!stop
  ...
  ! set up forcing function and boundary conditions
    ...
    allocate( uipert(ni,nj,nk) )
    allocate( vipert(ni,nj,nk) )

    allocate( dwdz(ni,nj,nk) ) 
    allocate( dudx(ni,nj,nk) ) 
    allocate( dvdy(ni,nj,nk) ) 
    allocate( dvdx(ni,nj,nk) ) 
    allocate( dudy(ni,nj,nk) ) 
    allocate( div(ni,nj,nk) ) 

    allocate( dwdx(ni,nj,nk+1) ) 
    allocate( dwdy(ni,nj,nk+1) ) 

    allocate( dum1(i,j,k) )
    allocate( dum2(i,j,k) )
    allocate( dum3(i,j,k) )

    ! compute 3D wind derivatives

    do i=1,ni
    do j=1,nj
    do k=1,nk
      uipert(i,j,k) = ui(i,j,k) - u0(k)
      vipert(i,j,k) = vi(i,j,k) - v0(k)
      dudx(i,j,k) = (upert(i+1,j  ,k) - upert(i,j,k))*rdz
      dvdy(i,j,k) = (vpert(i  ,j+1,k) - vpert(i,j,k))*rdz
      dwdz(i,j,k) = (w(i,j,k+1) - w(i,j,k))*rdz*mzh(k)
    ...
    do j=1,nj
    do k=1,nk
      dvdx(1,j,k) = (vipert(2,j,k) - vipert(1,j,k))*rdz
      dvdx(ni,j,k) = (vipert(ni,j,k) - vipert(ni-1,j,k))*rdz
      do i=2,ni-1
        dvdx(i,j,k) = 0.5*(vipert(i+1,j,k) - vipert(i-1,j,k))*rdz
    enddo
    enddo
    enddo
    ...
    do i=1,ni
    do k=1,nk
      dudy(i,1,k) = (uipert(i,2,k) - uipert(i,1,k))*rdz
      dudy(i,nj,k) = (uipert(i,nj,k) - uipert(i,nj-1,k))*rdz
      do j=2,nj-1
        dudy(i,j,k) = 0.5*(uipert(i,j+1,k) - uipert(i,j-1,k))*rdz
    enddo
    enddo
    enddo

    ! horizontal gradient of w on staggered w grid (used for term 3)

    do j=1,nj
    do k=1,nk+1
      dwdx(1,j,k) = (w(2,j,k) - w(1,j,k))*rdz
      dwdx(ni,j,k) = (w(ni,j,k) - w(ni-1,j,k))*rdz
      do i=2,ni-1
        dwdx(i,j,k) = 0.5*(w(i+1,j,k) - w(i-1,j,k))*rdz
    enddo
    enddo
    enddo
    ...
    do i=1,ni
    do k=1,nk+1
      dwdy(i,1,k) = (w(i,2,k) - w(i,1,k))*rdz
      dwdy(i,nj,k) = (w(i,nj,k) - w(i,nj-1,k))*rdz
      do j=2,nj-1
        dwdy(i,j,k) = 0.5*(w(i,j+1,k) - w(i,j-1,k))*rdz
    enddo
    enddo
    enddo
    ...
    do i=1,ni
    do j=1,nj
      dum1(i,j,1) = rho0(1)*wi(i,j,1)*(div(i,j,2) - div(i,j,1))* &
                    & rdz*mzf(2) + &
                    & 0.5*(drho0dz(1) + drho0dz(2))*wi(i,j,1)*dwdz(i,j,1)
      do k=2,nk
        dum1(i,j,k) = rho0(k)*wi(i,j,k)*(div(i,j,k) - div(i,j,k-1))* &
                      & rdz*mzf(k) + &
                      & 0.5*(drho0dz(k) + drho0dz(k+1))*wi(i,j,k)*dwdz(i,j,k)
      enddo
    enddo
    enddo
    ...
    do j=1,nj
    do k=1,nk
      dum2(i,j,k) = rho0(k)*( (dudx(i,j,k)*dudx(i,j,k)) + &
    ...
    allocate( wterms(ni,nj,nk+1) )

    wterms(:,:,:) = 0.0
    do i=1,ni
    do j=1,nj
    do k=2,nk
      wterms(i,j,k) = ( dwdx(i,j,k)*(uipert(i,j,k) - uipert(i,j,k-1))*rdz*mzf(k) + &
                      & dwdy(i,j,k)*(vipert(i,j,k) - vipert(i,j,k-1))*rdz*mzf(k) )
    enddo
    enddo
    enddo
    ...
    do j=1,nj
    do k=1,nk
      dum3(i,j,k) = 2.0*rho0(k)*( dvdx(i,j,k)*dudy(i,j,k) + &
                                & 0.5*(wterms(i,j,k+1) + wterms(i,j,k)) )
    enddo
    enddo
    enddo
    ...
    do j=1,nj
    do k=1,nk
      ff(i,j,k) = -dum1(i,j,k) - dum2(i,j,k) - dum3(i,j,k)
      !ff(i,j,k) = -dum1(i,j,k) - dum2(i,j,k)
    enddo
    enddo
    enddo
