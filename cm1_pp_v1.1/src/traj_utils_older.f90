...
contains
  ...
  !# v1.0, Ryan Hastings 13 December 2016                         #!
  !################################################################!
  subroutine interp_all( xpc, ypc, zpc, zzh, u, up, v, vp, w, wp, buoy, buoyp, &
    pgfx, pgfxp, pgfy, pgfyp, pgfz, pgfzp, &
    tke, tkep, thr0, thr, thrp, &
    qv, qvp, qc, qcp, qr, qrp, qi, qip, qs, qsp, qg, qgp, qhl, qhlp, &
    ncc, ncp, ncr, nrp, nci, nip, ncs, nsp, ncg, ngp, rhod, rhodp, &
    th, thp, ppi, pip )
    ...
    real,intent(in),dimension(1:ni+1,1:nj,0:nk) :: u, pgfx
    real,intent(in),dimension(1:ni,1:nj+1,0:nk) :: v, pgfy
    real,intent(in),dimension(1:ni,1:nj,1:nk+1) :: w, tke, pgfz

    real,intent(in),dimension(1:ni,1:nj,0:nk) :: buoy, thr, th, ppi
    real,intent(in),dimension(1:ni,1:nj,0:nk) :: qv, qc, qr, qi, qs, qg, qhl, rhod
    ...
    call interp_u( u, xpc, ypc, zpc, iif, jh, kh, zzh, pgfxp )
    call interp_v( v, xpc, ypc, zpc,  ih, jf, kh, zzh, vp )
    call interp_v( v, xpc, ypc, zpc,  ih, jf, kh, zzh, pgfyp )
